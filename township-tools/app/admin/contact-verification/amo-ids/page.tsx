...
type Summary = {
  ...
  unmatched: number;
  by_email: number;
  by_email_name: number;
  by_name_org: number;
};

export default function AmoIdAssignPage() {
  ...
  const submit = async (mode: "preview" | "commit") => {
    if (!file) return setError("Choose the AMO Individual Report file first.");
    setError(null);
    setLoading(true);
    try {
      ...
      fd.set("file", file);
      fd.set("mode", mode);
      const res = await fetch("/api/admin/contact-verification/amo-ids", { method: "POST", body: fd });
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-gray-100">
      <header className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-4 flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold">Assign AMO IDs</h1>
        </div>
      </header>
        ...
        {authorized && (
          <>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Upload the AMO <strong>Individual Report</strong> export. Each contact is matched by
              email, then name + organization, and assigned its AMO Individual ID. Preview first —
              committing only updates contacts whose ID changes.
            </p>
            ...
            {result ? (
              <section className="bg-white dark:bg-gray-900 border border-emerald-200 dark:border-emerald-900 rounded-xl p-6">
                ...
                <div className="text-sm text-gray-700 dark:text-gray-300 space-y-1 mb-4">
                  <div>
                    Updated <strong>{result.updated_count}</strong> contact
                    {result.updated_count === 1 ? "" : "s"} with an AMO ID.
                  </div>
                  ...
                  <div className="text-gray-500">
                    ...
                    not in AMO · {result.summary.ambiguous} ambiguous (left alone).
                  </div>
                </div>
                <button
                  type="button"
                ...
                {summary && (
                  <section className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm mb-4">
                      <Stat label="AMO rows" value={summary.amo_rows} />
                      <Stat label="Active contacts" value={summary.contacts} />
                      ...
                      <Stat label="Not in AMO" value={summary.unmatched} tone={summary.unmatched ? "amber" : "neutral"} />
                    </div>
                    <p className="text-xs text-gray-500 mb-4">
                      Match methods — email: {summary.by_email} · email+name: {summary.by_email_name} · name+org:{" "}
                      {summary.by_name_org}. Ambiguous and not-in-AMO contacts are left unchanged.
                    </p>
                    <div className="flex items-center gap-2">
                      ...
                      >
                        ...
                        Assign {summary.will_update} AMO ID{summary.will_update === 1 ? "" : "s"}
                      </button>
                      <button
                        type="button"
                        onClick={reset}
