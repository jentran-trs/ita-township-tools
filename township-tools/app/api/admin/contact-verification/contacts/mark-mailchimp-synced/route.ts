...
// POST — bulk mark (or unmark) contacts as synced to MailChimp.
// Body: { contactIds: string[], synced?: boolean }  (synced defaults to true)
// Stamps cv_contacts.mailchimp_updated_at + mailchimp_updated_by, writes one
// audit row per contact for traceability.
export async function POST(req: Request) {
  ...
  // Snapshot existing MailChimp state per contact so the audit row shows what changed.
  const { data: existing } = await supabase
    .from('cv_contacts')
    .select('id, township_id, mailchimp_updated_at, mailchimp_updated_by')
    .in('id', contactIds);

  const stampedAt = new Date().toISOString();
  const update = synced
    ? { mailchimp_updated_at: stampedAt, mailchimp_updated_by: adminDisplay }
    : { mailchimp_updated_at: null, mailchimp_updated_by: null };
  ...
  const auditRows = (existing || []).map((c: any) => ({
    ...
    contact_id: c.id,
    action: synced ? 'mailchimp_mark_synced' : 'mailchimp_unmark_synced',
    before: { mailchimp_updated_at: c.mailchimp_updated_at, mailchimp_updated_by: c.mailchimp_updated_by },
    after: update,
