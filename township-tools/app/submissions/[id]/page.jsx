      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-gray-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <button
            ...
            className="px-4 py-2 bg-amber-500 text-white rounded-lg font-medium hover:bg-amber-400"
          >
            Back to Projects
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-gray-100">
      ...
      <header className="sticky top-0 z-50 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-4">
          <div className="flex items-center gap-4">
            <button
              onClick={() => router.push('/projects')}
              className="flex items-center gap-2 px-3 py-1.5 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-lg transition-colors text-sm font-medium"
            >
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-8">
        {/* Cover Section */}
        <section className="bg-white dark:bg-gray-900 border-2 border-gray-200 dark:border-gray-700 rounded-2xl p-6">
          ...
          <div className="grid sm:grid-cols-2 gap-6">
            <div>
              <p className="text-xs text-gray-500 dark:text-gray-500 uppercase mb-1">Organization Name</p>
              <p className="font-medium">{submission.organization_name}</p>
            </div>
            <div>
            ...
            {submission.tagline && (
              <div className="sm:col-span-2">
                <p className="text-xs text-gray-500 dark:text-gray-500 uppercase mb-1">Tagline</p>
                <p className="text-gray-700 dark:text-gray-300 italic">"{submission.tagline}"</p>
              </div>
            )}
          </div>
          {submission.logo_url && (
        ...
        {submission.include_opening_letter && (
          <section className="bg-white dark:bg-gray-900 border-2 border-gray-200 dark:border-gray-700 rounded-2xl p-6">
            ...
            <div className="space-y-4">
              <div>
                <p className="text-xs text-gray-500 dark:text-gray-500 uppercase mb-1">Title</p>
                <p className="font-medium">{submission.letter_title}</p>
              </div>
              {submission.letter_subtitle && (
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-500 uppercase mb-1">Subtitle</p>
                  <p className="text-gray-700 dark:text-gray-300">{submission.letter_subtitle}</p>
                </div>
              )}
              {submission.letter_content && (
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-500 uppercase mb-1">Content</p>
                  <p className="text-gray-700 dark:text-gray-300 whitespace-pre-wrap">{submission.letter_content}</p>
                </div>
              )}
              <div className="flex gap-4 mt-4">
                {submission.letter_headshot_url && (
                  <div>
                    <p className="text-xs text-gray-500 dark:text-gray-500 uppercase mb-2">Headshot</p>
                ...
                {submission.letter_image1_url && (
                  <div>
                    <p className="text-xs text-gray-500 dark:text-gray-500 uppercase mb-2">Image 1</p>
                    <img src={submission.letter_image1_url} alt="Image 1" className="w-24 h-24 object-cover rounded-lg" />
                  </div>
                )}
                {submission.letter_image2_url && (
                  <div>
                    <p className="text-xs text-gray-500 dark:text-gray-500 uppercase mb-2">Image 2</p>
        ...
        {sections.length > 0 && (
          <section className="space-y-6">
            ...
            {sections.map((section, idx) => (
              <div key={section.id} className="bg-white dark:bg-gray-900 border-2 border-gray-200 dark:border-gray-700 rounded-2xl p-6">
                <h3 className="font-semibold mb-4">{section.title}</h3>

                {/* Stats */}
                {section.report_section_stats?.length > 0 && (
                  <div className="mb-4">
                    <p className="text-xs text-gray-500 dark:text-gray-500 uppercase mb-2 flex items-center gap-1">
                      <Hash className="w-3 h-3" />
                      Statistics
                    </p>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      {section.report_section_stats.map((stat, statIdx) => (
                        <div key={statIdx} className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3">
                ...
                {section.content && (
                  <div className="mb-4">
                    <p className="text-xs text-gray-500 dark:text-gray-500 uppercase mb-2">Content</p>
                ...
                {section.design_notes && (
                  <div className="mb-4">
                    <p className="text-xs text-gray-500 dark:text-gray-500 uppercase mb-2">Design Notes</p>
                    <p className="text-gray-600 dark:text-gray-400 italic">{section.design_notes}</p>
                  </div>
                )}
                      ...
                      {section.image_urls.map((url, imgIdx) => (
                        <div key={imgIdx} className="relative">
                          ...
                          {section.image_captions?.[imgIdx] && (
                            <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">{section.image_captions[imgIdx]}</p>
                          )}
                        </div>
                      ))}
                    </div>
            ...
            <div className="space-y-3">
              {submission.email && (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-gray-700 dark:text-gray-300">{submission.email}</span>
                </div>
              )}
              {submission.phone && (
                <div className="flex items-center gap-2">
                  <Phone className="w-4 h-4 text-gray-500 dark:text-gray-500" />
                  <span className="text-gray-700 dark:text-gray-300">{submission.phone}</span>
                </div>
              )}
            </div>
            <div className="space-y-3">
              {submission.street_address && (
                <div className="flex items-start gap-2">
                  <MapPin className="w-4 h-4 text-gray-500 dark:text-gray-500 mt-0.5" />
                  <div>
        ...
        {submission.additional_notes && (
          <section className="bg-white dark:bg-gray-900 border-2 border-gray-200 dark:border-gray-700 rounded-2xl p-6">
            ...
            <p className="text-gray-700 dark:text-gray-300 whitespace-pre-wrap">{submission.additional_notes}</p>
          </section>
        )}
      </main>
    </div>
